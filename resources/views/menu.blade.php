        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" style="background-color: #14b8a6">Pesan</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.querySelectorAll('.tambah-menu').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (orderItems[name]) {
                    ...
                    orderItems[name] = { name, price, quantity: 1 };
                }
                ...
                parent.innerHTML = `
                    ...
                    <span class="quantity-display font-14 fw-semibold mx-2">${orderItems[name].quantity}</span>
                ...
                attachQuantityControlListeners(parent, name, price);
            });
        });

        function attachQuantityControlListeners(parent, name, price) {
            parent.querySelector('.increase-quantity').addEventListener('click', function() {
                orderItems[name].quantity++;
                itemCount++;
                totalPrice += price;
                updateOrder();
                parent.querySelector('.quantity-display').textContent = orderItems[name].quantity;
            });
        ...
        showBasketButton.addEventListener('click', function() {
            ...
            Object.values(orderItems).forEach(item => {
                orderSummary.innerHTML += `<li>${item.name} x${item.quantity} - Rp ${new Intl.NumberFormat('id-ID').format(item.price * item.quantity)}</li>`;
            });
